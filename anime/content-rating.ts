...
const get = async () => {
  try {
    const data = await core.get('/content-rating')
    const $ = Cheerio.load(data.data)

    return $('main .loop li a').map((index, element) => ({
      id: $(element).attr('href')?.trim().replace('https://tenshi.moe/content-rating/', ''),
...
export const contentRatingAll = async (id: string, page?: number) => {
  if (!id) return { message: 'limit' }

  try {
    const data = await core.get(`/content-rating/${id}${page ? `?page=${page}` : ''}`, {
