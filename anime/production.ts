...
const get = async (page?: number) => {
  try {
    const data = await core.get(`/production${page ? `?page=${page}` : ''}`, {
    ...
    const list = $('main .loop li a').map((index, elemenet) => ({
      id: $(elemenet).attr('href')?.trim().replace('https://tenshi.moe/production/', '').replace(/\/[\d+]/gi, ''),
...
export const productionDetail = async (id: string) => {
  if (!id) return { message: 'limit' }

  try {
    const data = await core.get(`/production/${id}`)
    const $ = Cheerio.load(data.data)
...
export const productionAnime = async (id: string, page?: number) => {
  if (!id) return { message: 'limit' }

  try {
    const data = await core.get(`/production/${id}${page ? `?page=${page}` : ''}`, {
      withCredentials: true,
      headers: {
        Cookie: 'loop-view=thumb;'
      }
