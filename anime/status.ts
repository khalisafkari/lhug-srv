...
const get = async () => {
  try {
    const data = await core.get('/status')
    const $ = Cheerio.load(data.data)

    return $('main .loop li a').map((index, element) => ({
      id: $(element).attr('href')?.trim().replace('https://tenshi.moe/status/', ''),
...
export const statusAll = async (id: string, page?: number) => {
  if (!id) return { message: 'limit' }

  try {
    const data = await core.get(`/status/${id}${page ? `?page=${page}` : ''}`, {
